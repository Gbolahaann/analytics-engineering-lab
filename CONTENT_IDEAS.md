...
## 2026-08-18 — "A dbt test is just SQL you already write"
**Angle:** A `unique` test compiles to group by, count, having count > 1. A `relationships` test compiles to an anti-join looking for unmatched keys. Every BI analyst has written both a thousand times as ad-hoc QA before sending a dashboard out. The difference is that dbt runs them on every build, forever, instead of them living in one person's head and being forgotten the week they leave. Framed this way, testing stops being framework ceremony and becomes the thing analysts have always done, finally made permanent. Good de-intimidation post for BI people who think dbt is a new skill rather than their existing instincts formalised.
**Format:** short text post, optionally with the compiled SQL of a `unique` test screenshotted beside the two-word YAML that generated it. The contrast is the whole point.
